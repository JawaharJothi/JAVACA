				...
				<table class="table">
					<thead>
						<tr>
							...
							<th>#</th>
							<th><fmt:message key="label.grades.coursename" /></th>
							<th><fmt:message key="label.grades.credit" /></th>
							<th><fmt:message key="label.grades.grades" /></th>
								

						</tr>
					</thead>
					<tbody>
						<c:forEach var="courseGrade" items="${courseGradeList}" varStatus="status">
							<tr class="${status.index%2==0?'even':'odd'}">
								<td>${status.index + 1}</td>
								<td>${courseGrade.courseName}</td>
								<td>${courseGrade.credit }</td>
